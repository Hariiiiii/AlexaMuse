...
public class MainActivity extends Activity implements OnClickListener, TextToSpeech.OnInitListener {
    ...
    private void updateEeg() {
        ...
        if (eegBuffer[1] > 950 || eegBuffer[2] > 950) {
            System.out.println("Beta wave values EEG-Frontal lobe 1:" + eegBuffer[1] + ",EEG-Frontal lobe 2:" + eegBuffer[2]);
            speakOut("Your stress level is increasing", 1);

        }
        else if(eegBuffer[1]<=800 || eegBuffer[2]<=800){
            if(flag == 0) {
                speakOut("You seem so relaxed, do you want me to play some rock songs?", 0);
                startSpeechToText();
                flag = 1;
            }
        }
    ...
    private void speakOut(String t1, int i) {

        //String text = txtText.getText().toString();
        if (i == 1) {
            tts.speak(t1, TextToSpeech.QUEUE_FLUSH, null);
            SystemClock.sleep(5000);
            tts.speak("Alexa", TextToSpeech.QUEUE_FLUSH, null);
            SystemClock.sleep(1000);
            tts.speak("Play blues from spotify", TextToSpeech.QUEUE_FLUSH, null);
            SystemClock.sleep(5000);
            tts.speak("turn the temperature down and dim the lights", TextToSpeech.QUEUE_FLUSH, null);
            //SystemClock.sleep(2000);
            //tts.speak("Alexa, Play blues from spotify", TextToSpeech.QUEUE_FLUSH, null)
    ...
    private void startSpeechToText() {
        System.out.println("Inside speech module");
        ...
        switch (requestCode) {
            case SPEECH_RECOGNITION_CODE: {
                if (resultCode == RESULT_OK && null != data) {
                    ...
                    if(text.equals("yes")){
                        speakOut("Alexa play rock from spotify",2);
                    }
